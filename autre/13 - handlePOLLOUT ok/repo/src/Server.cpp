...
Server::Server(int argc, char** argv) : _hostname("irc.qhauuy-jteste.local")
{
	checkArgs(argc, argv);
		...
		if (_pollArray[i].revents)
		{
			if (_pollArray[i].revents & (POLLHUP | POLLERR | POLLNVAL))
			{
				if (_pollArray[i].revents & POLLNVAL)
					std::cout << "client (fd " << _clients[i]->getFd() << "): hard disconnect: invalid fd" << std::endl;
				else if (_pollArray[i].revents & POLLERR)
					std::cout << "client (fd " << _clients[i]->getFd() << "): hard disconnect: network error" << std::endl;
				else
					std::cout << "client (fd " << _clients[i]->getFd() << "): hard disconnect: disconnected" << std::endl;
				_clients[i]->setHardDisconnect();
				continue;
			}
